...
class EnricherTestCase(unittest.TestCase):
    ...
    def test_shared_ip_with_different_macs_never_merges(self):
        self._write('vip-a', _device('vip-a', mac='AA:BB:CC:00:00:0A', ip='192.0.2.20'))
        self._write('vip-b', _device('vip-b', mac='AA:BB:CC:00:00:0B', ip='192.0.2.20'))
    ...
    def test_dedup_remaps_references_to_kept_name(self):
        self._write('alpha', _device('alpha', mac='AA:BB:CC:00:00:01', ip='192.0.2.10'))
        self._write('host-10-10-0-10', _device('host-10-10-0-10',
                                               mac='AA:BB:CC:00:00:01', ip='192.0.2.10'))
        self._write('leaf', _device('leaf', ip='192.0.2.30',
    ...
    def test_physical_host_never_gains_hosted_on(self):
        ...
        self._write('router', _device('router', dtype='router', ip='192.0.2.1'))
        self._write('hv1', _device('hv1', dtype='hypervisor', ip='192.0.2.5',
                                   gateway='192.0.2.1',
    ...
    def test_container_gets_hosted_on_from_description(self):
        self._write('hv1', _device('hv1', dtype='hypervisor', ip='192.0.2.5'))
        self._write('ct101', _device('ct101', dtype='lxc', ip='192.0.2.50',
    ...
    def test_on_docker_is_not_a_parent(self):
        self._write('hv1', _device('hv1', dtype='hypervisor', ip='192.0.2.5'))
        self._write('svc', _device('svc', dtype='lxc', ip='192.0.2.60',
    ...
    def test_dns_and_gateway_links_created(self):
        self._write('router', _device('router', dtype='router', ip='192.0.2.1'))
        self._write('dns1', _device('dns1', dtype='server', ip='192.0.2.4',
                                    description='Technitium DNS server'))
        self._write('web', _device('web', ip='192.0.2.40',
                                   gateway='192.0.2.1', dns=['192.0.2.4']))
    ...
    def test_ambiguous_prefix_resolves_to_none(self):
        ...
        self._write('pve1', _device('pve1', dtype='hypervisor', ip='192.0.2.5'))
        self._write('pve2', _device('pve2', dtype='hypervisor', ip='192.0.2.6'))
        self._write('ct1', _device('ct1', dtype='lxc', ip='192.0.2.70',
...
class BmcPairingTestCase(EnricherTestCase):
    ...
    def _write_pairing_meta(self):
        (self.tmp / 'network.yml').write_text(
            'network:\n'
            '  name: testnet\n'
            '  bmc_pairing:\n'
            '    mgmt_prefix: "198.51.100."\n'
            '    main_prefix: "192.0.2."\n'
        )

    def test_configured_prefixes_link_bmc_to_host(self):
        self._write_pairing_meta()
        self._write('ipmi1', _device('ipmi1', dtype='bmc', ip='198.51.100.5'))
        self._write('hv1', _device('hv1', dtype='hypervisor', ip='192.0.2.5'))

        summary = enricher.enrich_dataset(self.tmp)

        self.assertEqual(summary['bmc_links'], 1)
        deps = self._read('ipmi1')['device']['dependencies']
        self.assertIn('hv1', deps['depends_on'])
        self.assertTrue(deps['bmc_pairing_inferred'])

    def test_without_config_the_heuristic_is_disabled(self):
        self._write('ipmi1', _device('ipmi1', dtype='bmc', ip='198.51.100.5'))
        self._write('hv1', _device('hv1', dtype='hypervisor', ip='192.0.2.5'))

        summary = enricher.enrich_dataset(self.tmp)

        self.assertEqual(summary['bmc_links'], 0)
        self.assertNotIn('depends_on',
                         self._read('ipmi1')['device'].get('dependencies', {}))
