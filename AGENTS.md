...
| `bin/env-sync.sh <env> <action>` | Sync `environments/<env>/` against its private repo (clone/pull/push/status; map in gitignored `.env-remotes`). Environment layers are single-branch — pull/push refuse from anything but the default branch |
