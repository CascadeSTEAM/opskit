#!/usr/bin/env python3
"""
ERPNext MCP Server — Frappe Helpdesk management across BMS and Cascade STEAM tenants.
...
Environment (or .env):
  ERPNEXT_ADMIN_PASSWORD_BMS=<password for helpdesk.bellinghammakerspace.org>
...
TENANTS = {
    "bms": {
        "site": "helpdesk.bellinghammakerspace.org",
        "description": "Bellingham Makerspace Helpdesk",
    },
    "cs": {
...
def test_tools():
    ...
    print("2. erpnext_list_tickets(tenant='bms', limit=3)")
    print(erpnext_list_tickets(tenant="bms", limit=3))
    print()

    print("3. erpnext_get_ticket(tenant='bms', ticket_id='0054')")
    print(erpnext_get_ticket(tenant="bms", ticket_id="0054"))
    print()
    ...
    print("6. erpnext_get_communications(tenant='bms', ticket_id='0054')")
    print(erpnext_get_communications(tenant="bms", ticket_id="0054"))
    print()

    print("7. erpnext_create_ticket(tenant='bms', ...)")
    ...
    if tid:
        print(f"\n8. erpnext_add_reply(tenant='bms', ticket_id='{tid}', ...)")
        print(erpnext_add_reply(tenant="bms", ticket_id=tid, content="<p>Test reply from MCP.</p>", reply_type="Comment"))
        print(f"\n9. erpnext_update_ticket(tenant='bms', ticket_id='{tid}', status='Closed')")
        print(erpnext_update_ticket(tenant="bms", ticket_id=tid, status="Closed", priority="Low"))
        print(f"\n10. erpnext_get_communications(tenant='bms', ticket_id='{tid}')")
        print(erpnext_get_communications(tenant="bms", ticket_id=tid))
    print()
    print("=== Tests passed ===")
