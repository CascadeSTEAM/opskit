#!/usr/bin/env python3
"""
Technitium DNS MCP Server — Zone, record, and DHCP management across YC, CS, and BMS servers.
...
Environment (or .env):
  ...
  TECHNITIUM_BMS_PASS=<password>           BMS DNS          (10.99.0.4)  — BW: dns-bms (TBD)
"""
...
SERVERS = {
    ...
    "bms": {
        ...
        "description": "Bellingham Makerspace DNS",
        "env_pass": "TECHNITIUM_BMS_PASS",
        "username": "admin",
    },
}
...
def dns_list_zones(server: str) -> str:
    ...
    Args:
        server: Server name — one of: cs-primary, cs-secondary, bms
    """
    try:
...
def dhcp_list_scopes(server: str) -> str:
    ...
    Args:
        server: Server name — one of: yeticraft, cs-primary, bms
    """
    try:
        client = get_client(server)
